...
export class AdminView implements OnInit, OnDestroy {
  ...
  persons: { uri: string; label: string }[] = [];
  linkSelection: Record<string, string> = {};
  ...
  loadPersons(): void {
    this.subscription.add(
      this.ontology.getClasses().pipe(
        switchMap(classes => {
          const cls = (classes || []).find(c => (c.uri || '').endsWith('#Person'));
          return cls ? this.ontology.getInstances(cls.uri) : of([]);
        })
      ).subscribe(insts => {
        this.persons = (insts || [])
          .map(i => ({ uri: i.uri, label: i.label || i.uri }))
  ...
  linkProfile(user: UserDto): void {
    const personUri = this.linkSelection[user.email] ?? '';
    if (personUri === (user.personUri || '')) return;
    this.linkError = null;
    this.linkSuccess = null;
    this.subscription.add(
      this.auth.linkUserPerson(user.email, personUri).subscribe({
        next: () => {
          this.linkSuccess = personUri
            ? `Profile linked for "${user.email}".`
            : `Profile unlinked for "${user.email}".`;
          this.cdr.markForCheck();
          this.loadUsers();
        },
        error: err => {
          this.linkError = err?.error?.error || 'Failed to link profile.';
          this.cdr.markForCheck();
        },
      })
    );
    ...
    this.subscription.add(
      this.auth.getUsers().subscribe({
        next: users => {
          ...
          this.linkSelection = {};
          users.forEach(u => {
            this.editingRoles[u.email] = u.role;
            this.linkSelection[u.email] = u.personUri || '';
          });
